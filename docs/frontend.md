...
Neo-brutalist: flat high-contrast colors (cream background, black borders, bold accent colors — yellow/pink/blue/green/orange/red), hard offset shadows with no blur, sharp corners (no rounded anything), chunky tactile buttons that shift on hover/press. Defined once in `app/globals.css` via Tailwind v4's `@theme` block (custom color/shadow tokens that become ordinary utility classes, e.g. `bg-neo-yellow`, `shadow-brutal`) plus a handful of `@layer components` primitives (`.neo-card`, `.neo-btn`, `.neo-input`, `.neo-badge`, `.neo-chip`) applied across every page instead of hand-repeating long utility strings. No new font or dependency — same Geist Sans/Mono already wired in `layout.tsx`, just leaning on heavier weights.
