...
- **Audit logging**: `AuditLogService` writes to `audit_log` for login attempts (`SUCCESS`/`FAILURE`), `GET /accounts/{id}`, `GET /settlements/{id}`, `POST /settlements` (`SUCCESS`/`DENIED` for the row-level check), and (Phase 4) `POST /reconciliation/runs` and `POST /reconciliation/mismatches/{id}/resolve`. **Known gap**: a role-check denial from `@PreAuthorize` (e.g. a `READ_ONLY` user attempting `POST /settlements` or anything under `/reconciliation/**`) happens in the AOP interceptor before the controller method body runs, so it is *not* currently audit-logged — only row-level denials inside a method body are. See Open Questions.
